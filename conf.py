...
def get_logdir(subdir):
    if subdir:
        return os.path.join(output_path, f"runs/new/{datetime.now().strftime('%Y.%m.%d')}/{subdir}/{datetime.now().strftime('%Y.%m.%d-%H:%M:%S')}")
    else:
        return os.path.join(output_path, f"runs/new/{datetime.now().strftime('%Y.%m.%d')}/{datetime.now().strftime('%Y.%m.%d-%H:%M:%S')}")
